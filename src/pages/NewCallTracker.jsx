...
function NewCallTracker() {
  ...
  // Update the state for OrderStatusForm data  
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentStage === "order-expected") {
        // Add order expected data
        rowData.push(
          orderExpectedData.nextAction,     // Column F - Next Action
          orderExpectedData.nextCallDate,   // Column G - Next Call Date
        )
        // Add empty values for remaining columns
        rowData.push(...new Array(25).fill(""))
      } else if (currentStage === "order-status") {
        // Add empty values for order expected columns
        rowData.push("", "")
        // Add order status
        rowData.push(orderStatusData.orderStatus) // Column H
        
        // Based on order status, add data to appropriate columns
        if (orderStatusData.orderStatus === "yes") {
          // Add YES data from the inline form
          rowData.push(
            orderStatusData.acceptanceVia,     // Column I
            orderStatusData.paymentMode,       // Column J
            orderStatusData.paymentTerms,      // Column K
            orderStatusData.transportMode,     // Column L
            orderStatusData.creditDays,        // Column M
            orderStatusData.creditLimit,       // Column N
            orderStatusData.orderVideo,        // Column O
            "", // acceptanceFileUrl placeholder  // Column P
            orderStatusData.orderRemark        // Column Q
          )
          // Add empty values for remaining columns
          rowData.push(...new Array(16).fill(""))
        } else if (orderStatusData.orderStatus === "no") {
          // Add empty values for yes columns
          rowData.push(...new Array(7).fill(""))
          // Add NO data
          rowData.push(orderStatusData.reasonForNo) // Column R - Reason for No
          // Add empty values for remaining columns
          rowData.push(...new Array(6).fill(""))
        } else if (orderStatusData.orderStatus === "hold") {
          // Add empty values for yes and no columns
          rowData.push(...new Array(8).fill(""))
          // Add HOLD data
          rowData.push(
            orderStatusData.holdingDate,  // Column S - Holding Date
            orderStatusData.holdRemark    // Column T - Hold Remarks
          )
          // Add empty values for remaining columns
        ...
        // Add empty values for all stage-specific columns
        rowData.push(...new Array(27).fill(""))
      }
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 border-t flex justify-between">
            ...
            {!(currentStage === "order-status" && orderStatusData.orderStatus === "yes") && (
              <button
                type="submit"
